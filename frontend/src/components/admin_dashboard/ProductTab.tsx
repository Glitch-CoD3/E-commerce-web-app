...
export type Product = {
  ...
  colors?: string[];
  sizes?: string[];
  images?: Record<string, string> | ProductVariantImage[];
  variants?: ProductVariant[];
};

// --- Local Form Variant Box Type ---
...
export default function ProductTab({
  ...
  // --- Dynamic Variant Box Management ---
  ...
  const removeVariantBox = (id: string) => {
    setForm(prev => ({
      ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        await axiosInstance.put(`/products/${editingId}`, updatePayload);
      } else {
        // --- CREATE FLOW ---
        ...
        // STEP 2 & 3: Loop through each Variant
        for (const variantBox of finalVariants) {

          // 🚨 FIXED FIELD NAMES HERE (color and size)
          const variantPayload = {
            product_id: Number(createdProductId),
            color: variantBox.colors || 'Standard', // Key is "color"
            size: variantBox.sizes || '',          // Key is "size"
          ...
          // STEP 3: Upload Image Files to /product-variants-images/:variantId
          // STEP 3: Compress & Upload Image Files
    ...
    return (
      <div className="space-y-8">
        ...
        <form onSubmit={handleSubmit} className={`${formSubBg} p-6 rounded-2xl border border-slate-800 space-y-6`}>
          ...
          {/* --- DYNAMIC VARIANT BOXES SECTION --- */}
          <div className="space-y-4 pt-2">
            <div className="flex items-center justify-between">
              <div>
                <h4 className="text-sm font-bold text-indigo-400 uppercase tracking-wider">Product Variant Boxes</h4>
                <p className="text-[11px] text-slate-400">Add different colors, sizes, prices, and upload dedicated images for each box.</p>
              </div>
              ...
              >
                <span>+ Add New Variant Box</span>
              </button>
            </div>

            {form.variants.length === 0 ? (
              <div className="p-4 rounded-xl border border-dashed border-slate-800 text-center text-xs text-slate-500">
                No custom variants added. (A standard single-product variant will be auto-generated on save).
              </div>
            ) : (
              <div className="grid grid-cols-1 gap-4">
                {form.variants.map((v, index) => (
                  <div key={v.id} className="p-4 rounded-xl border border-indigo-500/20 bg-slate-950/80 space-y-4 relative">
                    ...
                    {/* Embedded File Uploader for THIS Variant */}
                    <div className="space-y-2 pt-2 border-t border-slate-800/60">
                      <label className="block text-[11px] font-semibold text-slate-400">
                        Upload Images for this ({v.colors || 'Variant'}) Box
                      </label>
                      ...
                      {/* Variant Specific Images Preview */}
                      {v.images.length > 0 && (
                        <div className="flex gap-2 items-center flex-wrap pt-2">
                          {v.images.map(img => (
                            <div key={img.id} className="relative group">
                              ...
                              <button
                                type="button"
                                onClick={() => removeVariantImage(v.id, img.id)}
                                className="absolute -top-1.5 -right-1.5 bg-rose-600 text-white text-[10px] rounded-full w-4 h-4 flex items-center justify-center font-bold"
                              >
                                ×
                              </button>
                            </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              {isSaving ? 'Saving Product & Variants...' : editingId ? 'Update Product' : 'Save Product'}
            </button>
                  ...
                  return (
                    <tr key={p?.id ? `prod-${p.id}` : index} className={`hover:bg-indigo-500/5 ${borderRow}`}>
                      ...
                      <td className="p-3 text-xs font-mono text-indigo-400">
                        {p.total_variants ?? (Array.isArray(p.variants) ? p.variants.length : 0)} Option(s)
                      </td>
                        ...
                        <button
                          onClick={() => {
                            ...
                            if (Array.isArray(p.variants) && p.variants.length > 0) {
                              ...
                              const imagesObj = (p.images && typeof p.images === 'object' && !Array.isArray(p.images)) ? p.images : {};

                              mappedVariants = p.colors.map((color, idx) => {
                                const sizeVal = Array.isArray(p.sizes) ? (p.sizes[idx] || '') : '';
                                const colorImageUrl = imagesObj[color] || '';

                                return {
                                  id: `flat-var-${idx}`,
                                  colors: color,
                                  sizes: sizeVal as ProductSize | '',
                                  price: Number(p.price) || 0,
                                  stock_quantity: Number(p.stock_quantity) || 0,
                                  images: colorImageUrl ? [{ id: `img-${idx}`, previewUrl: colorImageUrl }] : [],
                                };
                              });
                            }
